...
// Komponen Paginasi Tabel yang diambil dari WABlastMessagesTable
// (Table Pagination component taken from WABlastMessagesTable)
...
const AddressTable: React.FC = () => {
  ...
  const NEXT_PUBLIC_WHATSAPP_SERVER_URL =
    ...
    "http://localhost:5000/api/whatsapp/address";

  const fetchAddresses = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const response = await fetch(NEXT_PUBLIC_WHATSAPP_SERVER_URL);
  ...
  const handleDelete = React.useCallback(
    async (id: string) => {
      try {
        const response = await fetch(
          `${NEXT_PUBLIC_WHATSAPP_SERVER_URL}/${id}`,
          {
            method: "DELETE",
          }
        );
    ...
    [NEXT_PUBLIC_WHATSAPP_SERVER_URL, fetchAddresses]
  );
  ...
  const handleSaveEdit = React.useCallback(
    async (id: string, newAddress: string, newName: string) => {
      try {
        const updatedData = { address: newAddress, name: newName };
        const response = await fetch(
          `${NEXT_PUBLIC_WHATSAPP_SERVER_URL}/${id}`,
          {
            method: "PUT",
    ...
    [NEXT_PUBLIC_WHATSAPP_SERVER_URL, setEditingRow, fetchAddresses]
  );
  ...
  const columns: ColumnDef<Address>[] = useMemo(
    ...
    [editingRow, handleDelete, handleSaveEdit]
  );
